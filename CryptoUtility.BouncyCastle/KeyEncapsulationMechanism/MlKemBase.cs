...
public abstract class MlKemBase : IKeyEncapsulationMechanism
{
    ...
    public (byte[] sharedSecret, byte[] ciphertext) Encapsulate(byte[] peerPublicKey)
    {
        ...
        var pubKey = (MLKemPublicKeyParameters)Org.BouncyCastle.Security.PublicKeyFactory.CreateKey(peerPublicKey);
    ...
    public byte[] Decapsulate(byte[] secretKey, byte[] ciphertext)
    {
        ...
        var privKey = (MLKemPrivateKeyParameters)Org.BouncyCastle.Security.PrivateKeyFactory.CreateKey(secretKey);
