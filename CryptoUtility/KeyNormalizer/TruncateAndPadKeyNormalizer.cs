...
public sealed class TruncateAndPadKeyNormalizer : IKeyNormalizer
{
    ...
    /// If the key is longer than <paramref name="keySizeBytes"/>, it is truncated.
    ...
    /// <returns>A byte array of length <paramref name="keySizeBytes"/> containing the normalized key.</returns>
    ...
    public byte[] Normalize(byte[] key, int keySizeBytes)
    {
        ...
        if (key.Length > keySizeBytes)
            Buffer.BlockCopy(key, 0, normalized, 0, keySizeBytes);
        else
